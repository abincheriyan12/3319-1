...
while True:
    ...
    ct = key.encrypt(message.encode('utf-8'), padding = True) #encrypt a msg
    print("*" *30)
    print("key is ", des_key)
    print("sent plaintext is: %s" % message)
    print("sent ciphertext is: %s" % ct.decode('utf-8', 'ignore'))
    print("*" *30)
    s.send(ct) #send ciphertext
    receive = s.recv(1024) #recieved the msg
    print("*" *30)
    print(" Recieved ciphertext is: %s" % receive.decode('utf-8', 'ignore'))
    pt = key.decrypt(receive, padding = True).decode() #decrypt the msg
